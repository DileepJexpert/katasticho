...
## Stack
- **Backend:** Spring Boot 3.5.16, Java 25, JPA/Hibernate, Flyway, PostgreSQL. Build: Maven (`./mvnw`).
...
- **Internationalization migrations (2026-06-22):** `V3__add_gulf_africa_currencies.sql` (OMR/BHD/KWD=3dp, SAR/QAR/KES=2dp) + `V4__coa_template_country.sql` (coa_template gains `country` col, backfill 'IN'; Gulf 'AE' TRADING template = India minus 9 statutory accts + VAT 2041/1511). See `docs/INTERNATIONALIZATION_PLAN.md`. **Latest migrations:** `V56` = biometric attendance TCP & ADMS cloud push; `V57` = multi-barcode packaging hierarchy (`item_packaging_barcode`); `V58` = job work & subcontracting Challan 45 / ITC-04 (`job_work_order`, `job_work_issue_line`, `job_work_receipt_line`); `V59` = offline POS sync partial unique index (`sales_receipt.offline_receipt_number`); `V60` = user-defined custom fields (UDF) framework (`custom_field_definition`, `custom_field_value`); `V61` = store shelf merchandising & photo audits (`store_merchandising_audit`). **Next new migration = V62**.
...
## Git / Workflow
- Active feature branch: `Codex/erp-requirements-doc-g0o1P`. Develop, commit, push here. Do NOT push elsewhere without permission.
...
- **`recordStockForBill` decision (option a — intentional fallback for direct
  ...
  (AGENTS.md) is "GRN is the only stock-posting step" — but small orgs that
...
- **`mcp/`** (TypeScript, not Java) — **MCP server** so Codex Desktop / agents can run the books via the REST API using an API key. Tools: ask, list_bills, list_invoices, list_ai_inbox, draft_bill, approve_bill_draft, reject_bill_draft. `mcp/README.md` has Codex Desktop setup. Drafts-only-until-approved.
...
#### Last-week audit-fix round (2026-07-04) — 6 confirmed defects across Codex + Codex commits
