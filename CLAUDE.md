...
- **Sidebar configurability:** every NavItem and NavGroup in `flutter_app/lib/routing/shell_screen.dart` carries a stable `id` (snake-case-dotted, e.g. `sales.invoices`). OWNER/ADMIN can hide entries via `org_settings.nav.disabled` (write via `PUT /api/v1/settings/nav.disabled` with a JSON array of ids) or the **Sidebar Customisation** settings screen at `/settings/nav-customisation`. PLATFORM_ADMIN role bypasses the disable list. Industry/role/country gates live on the NavItem fields directly (`roles`, `industries`, `countries` — null = no constraint). **The PRIMARY module gate is capability-based, NOT the NavItem `industries` field** (only ~3 of ~151 items set `industries`): `_isNavItemVisible(route, capabilities)` maps a route prefix → a `canUse*` flag in `flutter_app/lib/core/auth/business_capabilities.dart`. Capabilities = backend feature flags (`GET /api/v1/settings/features`) **OR-combined** with an industry `fallback`; an unmapped route falls through to `return true`. **2026-07-10 gating fix:** HR & Payroll (`/hr`,`/payroll`), Supply Planning (`/supply-chain`), Courier & Transport (`/courier`,`/transport`) were leaking to every vertical (no route→capability case). Added `canUsePayroll` / `canUseSupplyChain` / `canUseCourier` (from `PAYROLL` / `SUPPLY_CHAIN` / `COURIER`|`TRANSPORT` flags; fallback = `isDistributor || isManufacturer`, so a plain retailer no longer sees them — opt in per-org via the feature flag). Backend module gating (`@RequiresModule` + `ModuleAccessService`) is weak: billing enforcement off by default + **OWNER/ADMIN bypass the flag check for all non-"strict" modules**, so an OWNER can still reach hidden endpoints by API (frontend is the real visibility layer). **QA test login:** `TestAccountBootstrapService` (config-gated `app.test-account.enabled`, default OFF, env `TEST_ACCOUNT_ENABLED=true`) seeds ONE org + OWNER via the real `register()` path with EVERY module flag on + onboarding pre-completed — a known dev login (default phone `9000000001` / password `Test@12345`) that sees the whole app. Idempotent, never touches real signups; do NOT enable in production (known password). **Per-org authoritative override (2026-07-10):** `ModuleVisibilityService` + `/api/v1/settings/module-visibility` store a per-org `org_settings.modules.visibility` JSON map `{MODULE: bool}`; `BusinessCapabilities.applyModuleOverrides` applies it ON TOP of the flag+fallback so an explicit Show/Hide WINS in both directions (a feature-flag disable can't hide a fallback-on module; this can). ERP **Modules** screen `/settings/modules` (Settings tile + palette) = Default/Show/Hide per module, per org, zero cross-org impact. Full guide: `docs/MODULE_VISIBILITY_CONFIGURATION.md`; UAT: `docs/testing/07_SETTINGS_MODULES_test_cases.md`.
...
#### Phase-H review pass (2026-07-02) — adversarial workflow, 4 defects + 1 hardening
An adversarial 2-reviewer + per-finding verifier Workflow over the H1/H3 diffs confirmed 4 real defects (fixed before push) plus a money-safety hardening:
- **H1 webhook lost settlements on transient failure (high):** the public webhook always returned HTTP 200 even when processing threw, and a mid-processing runtime error (deadlock/lock-timeout) rolled back the whole @Transactional — dedupe row included — so Razorpay treated the event as delivered and never retried a payment it had captured. **Fix:** `handleWebhook` catches only deterministic `BusinessException` (→ 200 + dedupe row so a re-send stops); a transient infra exception now propagates, the txn rolls back (no dedupe row), and the controller returns 5xx so Razorpay retries.
- **H1 double-settlement TOCTOU (hardening):** the captured-payment + link-already-PAID checks were check-then-act with no covering lock, so two concurrent settlement events (Razorpay sends both `payment.captured` and `payment_link.paid`) for one capture could book two AR payments when the balance was ≥ 2× the payment. **Fix:** a `@Lock(PESSIMISTIC_WRITE)` re-read of the link before the settled-check serialises the two events; the `(provider, provider_payment_id)` unique index backstops the multi-link edge.
- **H3 generate-now foreign template (high):** `RecurringJournalService.generateFromTemplate` looked the template up with a non-org-scoped `findById`, so an admin could POST generate-now with another org's template id — posting a spurious journal into their own books AND advancing the victim org's cursor (silently skipping that org's scheduled entry). **Fix:** org-scoped `findByIdAndOrgIdAndIsDeletedFalse(id, currentOrg)` in both bill + journal generate (the daily job already sets TenantContext to each template's own org, so the sweep is unaffected).
- **H3 auto-post journal stall (medium):** an `auto_post` recurring journal whose post failed rolled back with no DRAFT and no cursor advance (asymmetric with the bill path). **Fix:** always create the entry as a DRAFT (`postJournal(autoPost=false)`) then best-effort `postEntry` — a post-specific failure leaves a valid DRAFT + advances the cursor.
- **H3 end-date < start-date (low):** create/update omitted the end≥start check the invoice mirror enforces, so an already-expired template still generated one document. **Fix:** `requireEndAfterStart` on create + update in both services.
Regression tests added for each: RecurringBillServiceTest +2, RecurringJournalServiceTest +3, PaymentLinkServiceTest +1 (transient-error propagation + no dedupe row).

#### Recurring bills + recurring journals (2026-07-02) — MASTER_GAP_TRACKER H3
- **V28 migration:** `recurring_bill` + `recurring_bill_generation` and `recurring_journal` + `recurring_journal_generation`, mirroring `recurring_invoice` exactly (JSONB line payload — no child line tables; a due-date cursor `next_bill_date`/`next_run_date`; status ACTIVE/PAUSED/STOPPED/EXPIRED; unique index on the generation link's document id).
- **`recurring/service/RecurringBillService`** (mirror of RecurringInvoiceService, Clock-injected): CRUD (STOPPED/EXPIRED not editable; resume snaps a past cursor to today), `generateFromTemplate` drafts a `PurchaseBill` from the JSONB lines via `PurchaseBillService.createBill` (DRAFT), auto-posts via `postBill` only when `auto_post` (in try/catch — a post failure leaves the DRAFT, never fails the sweep), writes a generation row, advances the cursor via `RecurringFrequency.advance`, flips EXPIRED past `end_date`. Guards: vendor contact required (VENDOR/BOTH), GOODS line needs an itemId (pre-empts AP_GOODS_ITEM_REQUIRED at generation), lines required.
- **`recurring/service/RecurringJournalService`:** same shape; `generateFromTemplate` posts a `JournalEntry` via `JournalService.postJournal(autoPost = template.auto_post)` — **DRAFT by default** (a wrong recurring journal posting into a closed/wrong period is worse than a stray DRAFT). Template lines are validated to **balance (Σdebit = Σcredit) and be non-zero at create/update**, so a generation run can never mint an imbalanced journal.
- **`recurring/job/RecurringDocumentJob`** — one `@Scheduled` (`app.recurring-document.cron` default 06:05) + `@SchedulerLock("RecurringDocumentJob")` sweeps both bills and journals; per-template it sets TenantContext (orgId + createdBy + SYSTEM) in try/finally and catches per-template so one bad template can't poison the batch (mirrors RecurringInvoiceJob). Distinct lock name from the invoice job.
- **Endpoints:** `/api/v1/recurring-bills` + `/api/v1/recurring-journals` — full CRUD + `/{id}/stop|resume|generate-now|generated-bills|generated-journals`, `@RequiresModule(RECURRING_BILLING)` (reuses the recurring-invoice module gate), same role matrix.
- **Tests:** RecurringBillServiceTest (7 — create active + cursor at start, non-vendor rejected, GOODS-without-item rejected, generate drafts + advances + logs, auto-post, skip non-active, EXPIRED flip) + RecurringJournalServiceTest (7 — balanced accepted, imbalanced/zero rejected, generate DRAFT + advance, auto-post, skip non-active, EXPIRED flip). **Flutter screens (clone recurring_invoices) = follow-up.**

#### Instalment payment terms + dunning levels (2026-07-03) — MASTER_GAP_TRACKER H2
- **Odoo parity, two AR-collections capabilities, PURELY ADDITIVE** — the single payment engine (`InvoiceService.updatePaymentStatus`, balanceDue = totalAmount − amountPaid) is untouched. New pkgs `com.katasticho.erp.paymentterm` + `com.katasticho.erp.dunning`. **V31 migration:** `payment_term` + `payment_term_line` (PERCENT/BALANCE + days_offset), `invoice_instalment` (materialised schedule), `dunning_level` (escalating stages), `dunning_log` (audit + idempotency; partial unique `(org, invoice, level) WHERE outcome='SENT'`).
- **Instalment terms:** `PaymentTermService` — CRUD; `computeSchedule` splits total across lines, **the final line takes the exact remainder** (Σ == total to the paisa; absorbs a trailing BALANCE line + rounding). `applyToInvoice(invoiceId, termId)` (only while `amountPaid == 0`) materialises `invoice_instalment` rows + sets `invoice.dueDate` to the FINAL instalment; **per-instalment paid/overdue status is DERIVED at read time** from the invoice's existing `amountPaid` via an oldest-first waterfall (`deriveStatus`) — no payment ever targets a dated sub-balance. Endpoints: `/api/v1/payment-terms` CRUD + `/api/v1/invoices/{id}/instalments` (apply/get/clear). Line seq is always the list position (never a client value) so it can't collide on `uq_invoice_instalment_seq`. Percent validation rejects sum > 100 strictly (else the remainder line could go negative).
- **Dunning:** `dunning_level` (seq, days_overdue threshold, channel EMAIL/WHATSAPP/AI_INBOX/NONE, message template `{{customer}}/{{amount}}/{{days}}/{{invoiceNumber}}/{{dueDate}}/{{orgName}}`). `DunningService.runForOrg` (non-transactional — pins TenantContext to the org) walks overdue invoices (**instalment-aware effective due date** = earliest-unpaid instalment) via `collectCandidates` (findOverdueInvoices ∪ instalment-due), picks the highest applicable not-yet-sent level, and delegates each to **`DunningDispatcher.dispatch` (REQUIRES_NEW, CLAIM-BEFORE-SEND)** — it inserts+flushes the SENT `dunning_log` row FIRST so the partial unique index arbitrates concurrent sweeps (no double-send), then delivers; a non-viable channel (no email) skips without claiming (retries), a not-delivered channel throws to roll back the claim. Per-invoice isolation means a claim collision can never poison the batch. AI_INBOX escalates an open DUNNING suggestion in place (never downgrades). ShedLock `DunningJob` (daily 10am, opt-out `dunning.enabled`). Endpoints `/api/v1/dunning/levels` CRUD + `/run` + `/preview` + `/log`.
- **Adversarial review (3-lens + verifier) fixed 3 defects before push:** (A, medium) concurrent manual `/run` + cron double-sent + the unique-index violation poisoned the whole `@Transactional` sweep — fixed by the REQUIRES_NEW claim-before-send dispatcher above; (B, low) duplicate client line-seq collided on `uq_invoice_instalment_seq` → 500 — fixed by always using the ordered index; (C, low) the ±0.01 tolerance accepted a >100 percent sum → negative final instalment — fixed by rejecting >100 strictly.
- **Tests:** PaymentTermServiceTest (15 — schedule remainder-on-last for percent/BALANCE/rounding, waterfall PAID/PARTIAL/PENDING + overdue, effectiveDueDate, apply guards, percent-sum + BALANCE-not-last validation, over-100 rejected, thirds accepted, distinct seq), DunningServiceTest (12 — highest-applicable, idempotency pre-filter, email/AI_INBOX-create/escalate, instalment-aware overdue, **concurrent-claim prevents double-send**, channel viability skip, level validation). V31 boot-verified (Flyway → 31, ddl-validate clean, endpoints 401). **Flutter payment-term + dunning UI = follow-up.** 1622 total backend pass.

#### Workflow Rules engine — custom automation (2026-07-03) — MASTER_GAP_TRACKER I2
- **The "custom workflow" gap** (Zoho workflow-rules / Odoo automated-actions parity): on a document event, evaluate org-defined CRITERIA and run ACTIONS. Rides the EXISTING domain-event bus — `WorkflowRuleDomainEventHandler.supports()` returns true for ALL event types, so the engine needs no new publish sites for itself. New pkg `com.katasticho.erp.workflow` (NOT `common.workflow` = the approval engine).
- **V30 migration:** `workflow_rule` (org-scoped, JSONB `criteria` + `actions`, `entity_type` + `trigger_event`, `match_type` ALL/ANY, `active` default false, `run_order`) + `workflow_rule_execution` (audit + idempotency; unique partial `(org, rule, event_id)`).
- **`WorkflowCriteriaEvaluator`** (pure): operators EQ/NE/GT/GTE/LT/LTE/CONTAINS/IN/IS_EMPTY/IS_NOT_EMPTY with numeric coercion (a BigDecimal snapshot field vs a String operand compares numerically, not lexically). `WorkflowFieldResolver` merges the event payload + `DocumentSnapshotService.getSnapshot` (full Response-DTO fields for the 10 money docs; falls back to payload when no snapshot).
- **`WorkflowRuleService.evaluate(event)`:** loads active rules for (org, entity, trigger); **sets `TenantContext` from `event.getOrgId()` for the duration of evaluation (try/finally, restore-after)** because the domain-event WORKER THREAD never populates TenantContext (the bus + existing handlers pass orgId explicitly) — the execution recorder (`org_id NOT NULL`), the field mutator, and `AiSuggestionService.requireOrgId` all read it, so without this the dedupe row never commits and matched EMAIL/WEBHOOK actions re-fire on every at-least-once retry (fixed in the Phase-I review pass). Per rule — skip if an execution row already exists for (rule, event) (retry-safe on the at-least-once bus), skip if criteria don't match (no side effects → safe to re-evaluate), else run actions + record ONE execution row via `WorkflowExecutionRecorder` (**REQUIRES_NEW** so the dedupe row commits even if another handler on the event later rolls back). Per-rule try/catch so one bad rule can't fail the event and starve the AI/e-invoice handlers.
- **Actions:** EMAIL (`{{field}}` templating → `EmailService.sendHtml`), WEBHOOK (JSON body signed HMAC-SHA256 hex `X-Katixo-Signature`, POSTed via the shared `gspRestTemplate`; **SSRF guard = resolve-and-inspect: https-only + `InetAddress.getAllByName` then reject any loopback/any-local/link-local(incl. 169.254 cloud-metadata)/site-local/multicast/CGNAT address** — catches decimal/hex-encoded IP literals, IPv6 loopback forms, and public hostnames resolving to internal ranges that a literal-prefix check missed; runs at save AND at fire), AI_SUGGESTION (idempotent via `existsOpenSuggestion`), FIELD_UPDATE (per-entity `WorkflowFieldMutator` SPI with an explicit soft-field **allowlist**, writes via the plain repository only — never a posting/void path; ships `InvoiceFieldMutator` allowing only `notes`/`termsAndConditions`; a field outside the allowlist is rejected at rule-save).
- **`WorkflowRuleController`** @ `/api/v1/workflow-rules` (OWNER/ADMIN writes): CRUD + `/{id}/toggle` + `/dry-run` (describes, never executes) + `/{id}/executions` log + `/metadata` (trigger events, operators, action types, field-update allowlists for the UI builder).
- **Usefulness lever:** added `domainEventPublisher.publish()` at `PurchaseBillService.postBill` (BILL_POSTED) + `PaymentService.postPayment` (PAYMENT_RECORDED) — before, the bus only emitted INVOICE_POSTED, so rules could only fire on invoice-post; now bills + payments trigger too (snapshot-then-publish, mirroring InvoiceService). **Flutter rule-builder UI = follow-up.**
- **Tests:** WorkflowCriteriaEvaluatorTest (7 — numeric coercion, EQ/NE/CONTAINS, IN comma-list, IS_EMPTY, ALL vs ANY, **LT/LTE on a missing field ≠ match**), WorkflowRuleServiceTest (15 — matching fires EMAIL w/ templating + records MATCHED, non-match no-op, event idempotency skip, AI_SUGGESTION idempotent, per-rule isolation on failure, WEBHOOK internal-host + non-https rejected, FIELD_UPDATE allowlist enforced at save + mutator called for allowed field, actions-required, **evaluate-on-worker-thread resolves org from event + restores TenantContext, WEBHOOK rejects ipv4/ipv6 loopback + cloud-metadata + decimal-encoded loopback**) + PaymentServiceTest +1 constructor (publisher). 1595 total backend pass.
- **Phase-I review pass (2026-07-03) — adversarial 3-lens + per-finding verifier Workflow over the I2 diff confirmed 10 findings collapsing to 3 distinct defects, all fixed before push:** (1) **worker-thread null TenantContext (high):** `evaluate()` ran on the domain-event worker where TenantContext is empty, so the execution dedupe row (`org_id NOT NULL`) never committed (idempotency backstop dead → EMAIL/WEBHOOK re-fire on every retry), AI_SUGGESTION threw `requireOrgId`, and FIELD_UPDATE looked the entity up with a null org — all fixed by setting `TenantContext` from `event.getOrgId()` in a try/finally; (2) **SSRF bypass (high):** the literal host-prefix check missed encoded IPs / IPv6 forms / DNS-to-internal — replaced with resolve-and-inspect (see Actions above); (3) **LT/LTE on missing field (medium):** the `Integer.MIN_VALUE` incomparable sentinel made `field < N` fire on documents lacking the field — guarded in the two magnitude comparators.

#### Last-week audit-fix round (2026-07-04) — 6 confirmed defects across Codex + Claude commits
A 7-lens adversarial audit Workflow (money-posting / migrations / jobs-concurrency / engines / security-tenant / payroll-gst / codex-blockers, each finding independently verified) over the whole week's diff (`b95c1e8..HEAD`, ~195 files, 12 migrations) confirmed **6/8** raw findings (2 dropped: a duplicate + a refuted "workflow FIELD_UPDATE lost on co-handler rollback" — refuted because the INVOICE_POSTED co-handlers `EInvoiceService`/`EwayBillService.detectForInvoice` + `scanPostedInvoice` catch all exceptions internally and "never throw", so they never mark the shared `processOne` tx rollback-only; no fix shipped). All 6 confirmed fixed with regression tests, full suite 1625→**1639** green:
- **(critical) SqlValidator cross-tenant credential read** (`ai/service/SqlValidator.java`): the JSqlParser rewrite (commit 3a81341) treated ANY no-`org_id` public table as a shared "platform reference table" and injected NO filter, guarded only by a 5-name denylist — so `refresh_token` / `password_reset_token` / `delegated_access_token` / `platform_admin` / cross-org `trading_partner` etc. were readable unfiltered via `POST /api/v1/ai/query`. **Fix: inverted to a `PLATFORM_REFERENCE_TABLES` ALLOWLIST** (drug/salt/manufacturer/hsn/gst_state/generic_sub/drug_interaction/currency/coa_template/pt_slab/lwf_rule/ai_model_registry); every other no-org table is now rejected (`throw unsafe`), restoring the pre-rewrite fail-closed posture. Tests: SqlValidatorTest +5 (allowlisted reference passes; refresh_token/platform_admin/trading_partner rejected; UNION-smuggle rejected).
- **(medium) 3-way-match posting gate killed the bill-as-stock-source flow** (`ap/match/ThreeWayMatchService.assertPostable` + `ap/service/PurchaseBillService.postBill`): Codex's `assertPostable` (commit 856c3f9) blocked ANY PO-backed non-MATCHED/non-BYPASSED line under the default `required=true` — including `NO_GRN`, the exact shape `recordStockForBill` books stock for (supplier-bill-first, PO linked, no GRN). **Fix: `assertPostable` now blocks only genuine variances (`POSTING_BLOCKING_STATUSES` = QTY_OVER/PRICE_HIKE/AMOUNT_MISMATCH); NO_GRN posts through** (QTY_OVER/PRICE_HIKE only arise once a GRN exists, and there recordStockForBill already skips the stock post, so gating the AP posting there is correct). Also wrapped `postBill`'s `match()` in the same best-effort try/catch `createBill` uses (a match hiccup no longer hard-fails posting). Tests: ThreeWayMatchServiceTest +2.
- **(medium) notification sweep jobs silently dropped their writes** (`automation/{DailySalesSummaryJob,OverdueBillJob,LowStockAlertJob}`): `@Transactional(readOnly=true)` + a `Notification` (UUID `@GeneratedValue` → in-memory id, INSERT deferred to flush) → readOnly's `FlushMode.MANUAL` never flushed → every daily-summary / overdue-bill / low-stock in-app notification was lost. **Fix: dropped `readOnly` (now plain `@Transactional`, matching the already-correct `ExpiryAlertJob`/`PaymentReminderJob`).** Test: NotificationJobTransactionTest (structural guard — run() never readOnly).
- **(medium) webhook SSRF missed IPv6 ULAs** (`workflow/service/WorkflowRuleService.isBlockedAddress`): `Inet6Address.isSiteLocalAddress()` only catches deprecated `fec0::/10`, so RFC 4193 `fc00::/7` (the modern IPv6 private range, e.g. `fd00::/8`) slipped the guard. **Fix: explicit `(b[0] & 0xFE) == 0xFC` check on 16-byte addresses.** Test: WorkflowRuleServiceTest +1.
- **(medium) Form 12BB authz bypass** (`payroll/controller/TaxDeclarationController`): submit/deleteDraft/downloadForm12BB gated with `if (me != null && !owner) ensureAdmin()` — a caller with NO linked Employee (`me == null`) skipped the admin gate entirely, so any authenticated non-admin could read/tamper any employee's Form 12BB (PAN, rent, deduction PII). **Fix: extracted `assertOwnerOrAdmin(d)` that forces `ensureAdmin()` when `me == null`.** Tests: TaxDeclarationControllerAuthzTest (4 — null-employee non-admin FORBIDDEN on submit + PDF, admin allowed, owner allowed).
- **(low) match() resurrected an inbox suggestion for an OVERRIDDEN bill** (`ap/match/ThreeWayMatchService.match`): with `match()` now re-run inside `postBill`, an OVERRIDDEN bill whose lines still compute EXCEPTION spawned a fresh `THREE_WAY_MATCH_EXCEPTION` AI-Inbox item on every post (createSuggestionIfAbsent only dedupes on OPEN). **Fix: skip suggestion creation when the bill was already OVERRIDDEN** (capture `wasOverridden` once, reuse for both the status-preserve and suggestion guards). Test: ThreeWayMatchServiceTest +1.

#### Period-end forex revaluation (2026-07-04) — MASTER_GAP_TRACKER H8
- **The feature:** restates OPEN foreign-currency AR invoices + AP bills to the closing rate as of a period-end date. Book base value = balanceDue(foreign) × the document's stored `exchangeRate` (foreign→base); restated = balanceDue × closing rate (via `CurrencyManagementService.getExchangeRate`); delta = restated − book. One consolidated journal on the as-of date — DR/CR AR (default AR account), DR/CR AP (default AP account), balancing Forex Gain/Loss `5500` (the same account realized forex uses) — plus an **auto-reversing journal on as-of+1**, so the settlement-time realized forex (which compares payment rate vs the ORIGINAL invoice rate) is never double-counted. Sign rules: AR is an asset (delta>0 ⇒ DR); AP is a liability (delta>0 ⇒ CR); forex line = `net = arDelta − apDelta` (net gain>0 ⇒ CR, loss ⇒ DR). `com.katasticho.erp.accounting.forex`.
- **V33:** `forex_revaluation_run` (unique `(org, as_of_date)` for idempotency + audit: ar_delta / ap_delta / net_gain_loss / doc count / reval + reversal journal ids).
- **`ForexRevaluationService`:** `preview(asOfDate)` (compute, no post), `revalue(asOfDate)` (post + record), `listRuns()`. Per-doc it skips base-currency docs, docs issued after the as-of date, zero-balance docs, docs with no book rate, and docs with no closing rate (each → a `warnings` entry). Endpoints @ `/api/v1/reports/forex-revaluation` (`/preview`, `/run`, `/runs`, OWNER/ADMIN/ACCOUNTANT).
- **Adversarial review (4-dimension find → per-finding verify Workflow, 4/5 confirmed, all addressed):**
  - **(high, SYSTEMIC) journal source_module CHECK crash — V34:** `journal_entry_source_module_check` (V1 baseline) only allowed 11 values; a NON-mocked INSERT with `FOREX_REVAL` throws a CHECK violation → every non-zero reval crashes. **Root cause is systemic:** a dozen+ features already post source_modules the CHECK rejects — `MANUFACTURING`, `FIXED_ASSET`, `AMORTIZATION`, `TALLY_IMPORT`, `RECURRING_JOURNAL`, `PAYROLL_PAYMENT`, `INTEREST_CHARGE`, `HR_OFFBOARDING`, `AI_ENTRY`, `YEAR_END_CLOSE`, `BANKING` (H4) — all latent-broken because every one of their tests mocks `JournalService`. **Fix: `V34` DROPs the stale CHECK** (source_module is a label set from code constants, not user input; the enum guard's only net effect was latent crashes). Empirically proven: pre-V34 `INSERT 'FOREX_REVAL'` → check violation; post-V34 all values insert. **This un-breaks H4 + the pre-existing latent features too.**
  - **(medium) zero-delta run locked the date:** a no-op run (rates not yet entered) recorded a run row, and the idempotency check then blocked any later real run for that date. **Fix:** a zero-delta run posts nothing AND records nothing (returns a synthetic response with null id) — only a run that posts a journal is recorded/locks the date.
  - **(medium, scope-documented) current-balance not as-of-date state:** revaluation uses each document's CURRENT open balance, not the historical balance reconstructed as of the date (no payment/credit-note replay). Run at/near period end for accuracy; the auto-reversal confines any residual to the as-of-dated snapshot. Made explicit in the service javadoc; full as-of-date reconstruction is a documented follow-up.
- **Tests:** ForexRevaluationServiceTest (9 — AR gain DR/CR + reversal, AR loss, AP loss, AR-gain+AP-loss consolidated-balances, base-currency skip, no-closing-rate skip+warning, zero-delta no-lock, already-run idempotency, preview-no-post). Boot-verified: Flyway applied all 34 migrations + `ddl-validate` clean; V34 CHECK-drop empirically verified. **Flutter reval screen = follow-up.**
- **Next new migration = V35.**

#### User-defined bank matching rules (2026-07-04) — MASTER_GAP_TRACKER H4
- **The gap:** `BankReconciliationService` only scored a transaction against outstanding invoices (credits) / bills (debits); non-document transactions — bank charges, utility auto-debits, interest credits, salary transfers — never matched and sat UNMATCHED forever. A `bank_rule` lets an org categorise them to a GL account (QuickBooks/Xero "bank rules" parity).
- **V32 migration:** `bank_rule` (org-scoped, soft-delete: name, priority, active, direction_filter [ANY/CREDIT/DEBIT], narration_op [CONTAINS/EQUALS/STARTS_WITH/REGEX/ANY], narration_value, amount_op [ANY/EQ/GT/LT/GTE/LTE], amount_value, account_code, contact_id, memo, auto_apply — CHECK constraints on the three enums agree exactly with the service `Set`s) + `ALTER payment_match ADD account_code + bank_rule_id` (a new `ACCOUNT` match_type alongside INVOICE/BILL). **Next new migration = V33.**
- **`BankRuleService`:** CRUD (OWNER/ADMIN/ACCOUNTANT via `BankRuleController` @ `/api/v1/bank-rules`); `firstMatch(orgId, tx)` returns the first ACTIVE rule (ascending priority, then creation order) whose direction/narration/amount conditions ALL hold; `matches()` operators are null-safe + case-insensitive. Validation rejects bad enums, empty narration value for a non-ANY op, uncompilable regex, negative/absent amount for a non-ANY op, an account_code that doesn't exist for the org, and over-length name/memo.
- **Reconciliation integration:** in `importRows` + `rerunMatches`, a rule match takes precedence and SKIPS invoice/bill scoring (categorisation, not a document payment) → an `ACCOUNT` PaymentMatch (accountCode + bankRuleId, confidence 0.99). `acceptMatch` gains an ACCOUNT branch → `postAccountJournal`: CREDIT (money in) DR bank / CR account; DEBIT (money out) DR account / CR bank, via `JournalService.postJournal(autoPost=true)`; bank leg = chosen bankAccountId's GL code or the default BANK (1020). `autoApply` rules post on import.
- **Adversarial review pass (4-dimension find → per-finding verify Workflow, 10/12 confirmed, all fixed before push):**
  - **(high) rerunMatches double-post:** re-running a MATCHED (journal-posted) transaction deleted only SUGGESTED matches, re-suggested a fresh one, reset status to SUGGESTED — defeating the accept-time MATCHED guard → a second journal on re-accept. **Fix:** rerunMatches throws `BANK_TX_ALREADY_MATCHED` for a MATCHED transaction.
  - **(high) reference-less re-import double-post:** import dedup (`existsByOrgIdAndUtrAndDirection`) only fires for rows WITH a reference; a reference-less charge re-imported → autoApply re-posted its journal. **Fix:** autoApply posts ONLY when the row carries a reference (dedup key); reference-less matches fall back to a SUGGESTED match so a human catches re-import duplicates.
  - **(high) accept TOCTOU:** two concurrent accepts both passed the check-then-act MATCHED guard → double post. **Fix:** `acceptMatch` re-reads the transaction with `@Lock(PESSIMISTIC_WRITE)` (`findByIdAndOrgIdForUpdate`) so concurrent accepts serialise; the second reads MATCHED and is rejected.
  - **(medium) accept ignored the match's own status:** a REJECTED/IGNORED/ACCEPTED match could still be accepted. **Fix:** `acceptMatch` throws `BANK_MATCH_NOT_SUGGESTED` unless the match is still SUGGESTED.
  - **(medium) auto-apply aborted the whole import:** an un-postable auto-apply (target GL deleted after rule creation) threw out of the @Transactional import → the entire statement rolled back. **Fix:** the autoApply `acceptMatch` is wrapped in try/catch → degrades to a SUGGESTED match, import continues (the GL check throws BEFORE `postJournal`, so the import tx is never poisoned).
  - **(medium) regex ReDoS:** a valid-but-catastrophic rule regex (e.g. `(a+)+$`) executed per imported row could hang the import. **Fix:** `regexFind` runs the matcher over a `BoundedCharSequence` that throws once the engine reads > 200k chars, bounding backtracking to O(n) and treating a blow-up as no-match.
  - **(low) over-length name/memo → 500:** unvalidated length → DataIntegrityViolation → 500. **Fix:** service validates name ≤150 / memo ≤255 → clean 400.
  - **Refuted (no fix):** "no reversal/undo path" (journals reverse via the journal module; consistent with existing accepted-match behaviour) and "amountValue scale divergence" (the stated scenario doesn't occur).
- **Tests:** BankRuleServiceTest (13 — operators, firstMatch, validation, name-length, ReDoS-bounded) + BankReconciliationServiceTest +9 (ACCOUNT credit/debit journal shape, missing-GL throw, autoApply post + no-reference fallback + deleted-GL fallback, non-SUGGESTED-accept + MATCHED-rerun rejection). Boot-verified: Flyway applied all 32 migrations + `ddl-validate` clean (1661 total backend pass). **Flutter bank-rules CRUD screen = follow-up (no Flutter SDK in this env).**
