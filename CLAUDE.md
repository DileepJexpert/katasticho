...
- **Endpoints** (`/api/v1/fssai`): `GET /items/{id}`, `PUT /items/{id}` (OWNER/ADMIN/OPERATOR), `GET /allergens`, `GET /reports/allergen-exposure?allergen=...`, `GET /reports/license-renewal?daysAhead=60`.
- **Flutter:** `FssaiScreen` (`/inventory/fssai`, command palette "FSSAI Compliance" — keywords fssai/food/allergen/veg/license/label/codex/nutrition) — 3 tabs: Item compliance (paste an item id, edit veg dropdown with 🟢🔴🌱🟡 emojis, shelf-life days, date-marking dropdown, allergens as FilterChips populated from `/allergens`); Allergen exposure (pick allergen → table of items declaring it); License renewal (horizon dropdown 30/60/90/180 days → list of orgs+items expiring, urgent ≤30 days in red).
- **Tests:** `FssaiServiceTest` (12) — allergen codes normalised to upper-case incl. trimming whitespace, invalid 14-digit license throws, valid 14-digit license accepted, invalid veg class throws, invalid date marking throws, non-positive shelf life throws, exposure report case-insensitive on input + returns matching items + handles unknown allergen + blank guard, renewal returns org FBO row within horizon + omits beyond horizon, majorAllergens returns 14 sorted codes. 897 total backend pass.
