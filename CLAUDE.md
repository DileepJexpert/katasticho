...
## Courier / Shipping Integration — COD + RTO/Returns (DONE 2026-06-17, V25, module `com.katasticho.erp.courier`)
**The thinnest pillar (Transport) is now closed for the courier-fulfilment case.**
- **V25 tables:** `courier_shipment` (one parcel: partner/AWB/lifecycle/COD/freight/RTO timestamps) + `courier_shipment_event` (append-only status feed from webhook/poll/manual) + `cod_remittance` (one courier payout: gross/fees/net/expected/variance) + `cod_remittance_line` (per-AWB lines w/ match_status PENDING|MATCHED|AMOUNT_MISMATCH|ORPHAN). `ModuleCode.COURIER` added.
- **`CourierShipmentService`** — `create()` (DRAFT or BOOKED if AWB supplied; partner whitelist BLUEDART|DELHIVERY|INDIA_POST|DTDC|SHIPROCKET|OTHER), `markBooked()`, `cancel()` (DRAFT/BOOKED only), `recordEvent()` (DELIVERED stamps deliveredAt; RTO_DELIVERED back-fills rtoInitiatedAt; EXCEPTION informational), `pendingRtoReversal()` lists parcels needing manual credit-note + restock (the agent never auto-reverses books).
- **`CodReconciliationService`** — the genuinely new accounting loop. `create()` ingests a remittance + lines, pre-matches AWBs to shipments, computes gross/fees/expectedNet/variance (≠ 0 = courier short/over-remitted). `reconcile()` walks lines: **MATCHED** (AWB→invoice, COD = balance ±₹1) calls `PaymentService.recordPayment` with `COD_COLLECTION` method, stamping shipment.codRemittanceLineId; **AMOUNT_MISMATCH** flags w/o posting; **ORPHAN** raises a HIGH `COD_ORPHAN_AWB` AiSuggestion entityType=COD_REMITTANCE_LINE. Re-runs idempotent on already-MATCHED lines. **Fees are captured but NOT auto-posted** — same conservative rule as everywhere else; owner books DR Courier-Fee-Expense CR Bank against the courier's monthly fee invoice.
- **`CourierClient`** — provider-agnostic aggregator client mirroring `GspClient` exactly. Per-partner settings under `courier.<partner>.enabled|base_url|token|book_path|track_path|cod_remittance_path`. `bookShipment()` / `trackShipment()` / `fetchCodRemittance()` + `testConnection()` (any HTTP status = reachable, transport error = unreachable; never throws). Inert until creds set — manual AWB entry / remittance entry is the always-available fallback.
- **Endpoints:** `/api/v1/courier/shipments` (CRUD + `/book`, `/cancel`, `/events`, `/pending-rto`), `/api/v1/courier/cod-remittances` (create + `/{id}/reconcile`), `/api/v1/courier/settings` (per-partner GET/PUT + `/{partner}/test`). All `@RequiresModule(COURIER)`.
- **ERP Flutter:** new sidebar "Courier" group (Shipments + COD Remittances) with status filter + headline "X parcels need credit note + restock" RTO banner; COD remittance list w/ create-and-reconcile flow (quick-entry form: partner/bank/UTR/N AWB lines → save → reconcile with one tap → snackbar shows matched/mismatch/orphan counts); Settings → "Courier Connections" screen w/ per-partner enable + base URL + write-only token + endpoint paths + Test button (mirrors `GspSettingsScreen`). Command palette: Courier Shipments, COD Remittances, Courier Connections.
- **Tests:** CourierShipmentServiceTest (9) + CodReconciliationServiceTest (5). Full suite 849 pass; V25 boot-verified clean.
- **What's deliberately NOT built (matches "agent never auto-posts" rule):** RTO does not auto-issue credit note or auto-restock — pendingRtoReversal() surfaces the bucket, owner chooses cancel-invoice vs. credit-note via existing AR screens. COD fees per line are recorded but not posted — owner books them against the courier's monthly fee invoice. Variance on a remittance is visible, not silently posted to clearing.

## Lorry Receipt + Freight (road/GTA transport) — DONE 2026-06-17, V26, module `com.katasticho.erp.transport`
**Closes the rest of the Transport pillar — the truck/transporter (not parcel-courier) case the audit flagged: LR/consignment note + transporter rate cards + freight→AP billing.**
- **V26 tables:** `freight_rate_card` (per transporter + lane + weight slab: PER_KG|FLAT|PER_UNIT rate, min-charge, effective dates) + `lorry_receipt` (LR no./date, transporter [Contact VENDOR], vehicle/driver, route, packages/weight, freight + basis [PAID|TO_PAY|TO_BE_BILLED] + GST treatment [RCM|FORWARD|EXEMPT], links to DC/invoice/eway, freight_bill_id). `ModuleCode.TRANSPORT`.
- **`FreightRateCardService`** — CRUD + `resolveRate(transporter, origin, dest, mode, weightKg)`: picks the active card matching lane (case-insensitive; blank lane = wildcard) + weight slab (narrowest defined slab wins), computes PER_KG×weight w/ min-charge floor / FLAT / PER_UNIT. Drives LR freight auto-fill.
- **`LorryReceiptService`** — create (auto-fills freight from a rate card when amount omitted), lifecycle DRAFT→ISSUED→DELIVERED (+CANCELLED, blocked once freight billed). **`billFreight()`** raises a **DRAFT** purchase bill to the transporter via `PurchaseBillService` — SERVICE line on **HSN 9965 (GTA)** at the LR's GST rate, `reverseCharge=true` when RCM, freight account from org setting `transport.freight_account_code` (default 5240, falls back to the default PURCHASE account if absent). Only PAID/TO_BE_BILLED basis is billable (TO_PAY is the consignee's cost); idempotent (LR_ALREADY_BILLED). Reuses the existing AP path so GST/RCM/TDS-194C all apply on post — **the agent raises the draft, the owner posts it.**
- **Endpoints:** `/api/v1/transport/lorry-receipts` (CRUD + `/issue`, `/deliver`, `/cancel`, `/bill-freight`) + `/api/v1/transport/rate-cards` (CRUD + `/quote`). All `@RequiresModule(TRANSPORT)`.
...
## Courier Live Tracking — Shiprocket/aggregator adapter (DONE 2026-06-18, no migration)
**Makes the V25 courier loop live:** status from the aggregator (poll OR webhook) flows through the same `CourierShipmentService.recordEvent`, and COD payouts pull into a DRAFT remittance — provider-agnostic, inert until creds set (manual entry still works).
- **`CourierStatusMapper`** (pure, tested): aggregator free-text status → canonical event vocab (PICKED_UP/IN_TRANSIT/OUT_FOR_DELIVERY/DELIVERED/RTO_INITIATED/RTO_DELIVERED/EXCEPTION) or null=ignore. RTO checked before DELIVERED; EXCEPTION (incl. "UNDELIVERED" which *contains* "delivered") before DELIVERED; ignores NEW/pickup-scheduled/cancelled.
- **`TrackingPayloadParser`** (tolerant): extracts {awb,status,timestamp,location} from both the flat webhook shape (`current_status`+`awb`) and the nested Shiprocket track-API shape (`tracking_data.shipment_track[]` / `shipment_track_activities[]`).
- **`CourierTrackingService`**: `applyTracking` (the shared core) resolves the shipment by AWB, maps status, and **de-dupes** — skips a poll reporting the same status, never moves past a terminal state (DELIVERED/RTO_DELIVERED/CANCELLED). `syncShipment(id)` (manual poll), `syncOrg()` (bulk poll of in-flight shipments), `ingestWebhook(partner,payload)`, `resolveOrgByWebhookToken`, `pullCodRemittance(partner,from,to)` (tolerant COD parse → `CodReconciliationService.create` DRAFT).
- **Webhook routing (multi-tenant):** per-org token via `CourierClient.ensureWebhookToken` (stored `courier.<partner>.webhook_token`, reverse-looked-up via new `OrgSettingsRepository.findFirstByKeyAndValue`). Public `POST /api/v1/courier/webhooks/{partner}/{token}` (permit-all in SecurityConfig) resolves org → sets TenantContext → ingests → always 200 (no retry-storm, no leak).
- **`CourierTrackingJob`** (cron `app.automation.courier-tracking.cron` default every 30 min) polls in-flight shipments per active org — the backstop for couriers without webhooks. Inert for orgs with no courier configured.
- **Authenticated endpoints** (`/api/v1/courier/tracking`): `POST /shipments/{id}/sync`, `POST /sync-all`, `POST /cod-remittances/pull?partner=&from=&to=`, `GET /webhook-url?partner=`.
- **ERP Flutter:** shipment list gets a "Sync tracking" app-bar action (cloud_sync → syncAll → "X updated"); COD list gets a "Pull" action (partner picker → DRAFT remittance); courier settings partner card gets a "Webhook URL" button (fetch + copy the per-org path to paste into the aggregator dashboard).
- **Tests:** CourierStatusMapperTest (4) + CourierTrackingServiceTest (7 — advance/no-op/terminal-guard/unknown-or-missing/webhook-ingest/token-resolve/syncOrg-skips-terminal). Full suite 871 pass; boot-verified (webhook public→200 on bad token, tracking 401-secured).
...
2. ~~**Shiprocket adapter (live)**~~ DONE 2026-06-18 — see "Courier Live Tracking" section. Webhook receiver (per-org token routing) + poll job + COD-remittance pull, all through the existing `CourierShipmentService`/`CodReconciliationService`. Provider-agnostic; manual entry still the fallback.
3. ~~**Vehicle maintenance / fuel log**~~ DONE 2026-06-18 (V27, `transport`): `vehicle_log` (FUEL|SERVICE|REPAIR|INSURANCE|FITNESS|PERMIT|TYRE|OTHER per vehicle_number/van) + `VehicleLogService.summary` = total + by-type spend, distance (max−min odometer), **₹/km** + **km/litre mileage**. Endpoints `/api/v1/transport/vehicle-logs[/summary]`. ERP `VehicleLogScreen` (vehicle filter → TCO card + entries + add-entry sheet). Pure operational ledger — actual expense still booked via normal flow. Tests: VehicleLogServiceTest (3).
4. ~~**Proof-of-delivery (POD)**~~ DONE 2026-06-18 (V27, `transport`): `proof_of_delivery` (recipient/phone/GPS/deliveredAt, linked to DC/courier/invoice) + signature/photo via the shared `AttachmentService` (entityType POD). `POST /proof-of-delivery` + `/{id}/attachments` (multipart). ERP `ProofOfDeliveryScreen` (record POD + FilePicker→multipart photo/signature upload). Tests: ProofOfDeliveryServiceTest (3 — link-required, defaults, attach-via-AttachmentService).
