...
## PLANNED: Courier / Shipping Integration — COD + RTO/Returns (REQUIREMENT 2026-06-17, NOT BUILT)
**Why:** SMBs increasingly fulfil via third-party courier (BlueDart, Delhivery, India Post, DTDC, Ekart) or an aggregator (Shiprocket/NimbusPost/iThink) rather than own vans. The Transport pillar is the thinnest in the system (see prior audit) — this is the highest-value piece to close it. Distributor/D2C pain: track the parcel, collect & reconcile COD, and re-adjust stock + books when a parcel is returned (RTO).
**Scope (build on existing infra, don't fork):**
1. **Courier dispatch + tracking** — a `CourierShipment` (extend/parallel `supplychain/entity/Shipment.java`): courier partner, **AWB/tracking number**, service type, weight, declared value, status feed PICKED_UP→IN_TRANSIT→OUT_FOR_DELIVERY→DELIVERED→RTO_INITIATED→RTO_DELIVERED. Created from a `DeliveryChallan` (dispatch) or directly from an invoice. Status via aggregator webhook OR poll job (mirror the GSP pull cadence).
2. **Courier as a vendor + freight** — transporter/courier is a `Contact` (VENDOR); courier freight booked to AP (purchase bill / debit note), reconcilable against the courier's monthly invoice. Feeds landed cost where applicable (reuse `StockReceiptService.apportionLandedCost` pattern for inbound).
3. **COD lifecycle** — flag order/invoice `COD`; the courier collects cash from the customer and **remits to the seller minus COD fee**. On the remittance file/webhook: settle the invoice via `PaymentService.recordPayment` (DR Bank/COD-control, CR AR), book the COD fee as expense, and reconcile remitted-vs-expected (a COD control account, like a payment-gateway clearing account). Partial/failed remittance surfaces as an exception (AI Inbox suggestion, like ITC-at-risk).
4. **Returns / RTO → inventory + books re-adjust** — when a parcel is returned or RTO'd: create a **sales return → credit note** (`CreditNoteService`) that reverses the invoice (or cancels it if unbilled) AND a **restock stock movement** (reuse `return_order`/`ReturnOrderService` + the append-only reverse-movement rule, never UPDATE/DELETE). COD orders that RTO simply never settle — auto-close the COD expectation. Inventory rebuilds from the ledger as usual.
5. **Pluggable multi-courier integration** — provider-agnostic, **exactly mirror `GspClient`**: per-org `org_settings` (`courier.<provider>.enabled|base_url|token|...`), a `CourierClient` interface with adapters (BlueDart, Delhivery, India Post, DTDC) or a single aggregator adapter (Shiprocket-style) that fans out to many couriers. Inert until creds set; manual AWB entry is the always-available fallback (like the GSP manual-upload path). Settings screen + test-connection, same as `GspSettingsScreen`.
**Reuse map:** `Shipment`/`ShipmentService` (lifecycle), `DeliveryChallan` (dispatch source), `return_order`/`ReturnOrderService` + `CreditNoteService` (RTO → credit note + restock), `PaymentService` (COD settle), `InventoryService` (reverse-movement restock), `GspClient`+`GspSettingsScreen` (the provider-config + test-connection template), `EwayBillService` (already supplies the transport doc for ≥₹50k). **Net-new:** AWB-status sync (webhook/poll) + COD remittance reconciliation (the genuinely new accounting loop) + courier rate/fee handling. Next migration = V25.
