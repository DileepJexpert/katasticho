...
- **Tests:** WorkflowCriteriaEvaluatorTest (6 — numeric coercion, EQ/NE/CONTAINS, IN comma-list, IS_EMPTY, ALL vs ANY), WorkflowRuleServiceTest (10 — matching fires EMAIL w/ templating + records MATCHED, non-match no-op, event idempotency skip, AI_SUGGESTION idempotent, per-rule isolation on failure, WEBHOOK internal-host + non-https rejected, FIELD_UPDATE allowlist enforced at save + mutator called for allowed field, actions-required) + PaymentServiceTest +1 constructor (publisher).
