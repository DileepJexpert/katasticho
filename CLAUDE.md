...
## Flyway Migrations
- Location: `src/main/resources/db/migration/`. **Re-squashed 2026-06-20** to just two files — `V1__baseline_schema.sql` (full schema, CREATE-only) + `V2__seed_reference_data.sql` (drug/salt/manufacturer/HSN masters, GST state codes, coa_template, currency, generic_substitution, drug_interaction, ai_model_registry — all platform-level reference data, no org rows). **Next new migration = V3.**
