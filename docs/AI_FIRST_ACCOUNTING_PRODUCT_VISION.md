...
| 6 | **Tally parity backlog** (doc §1 order): ~~landed cost~~ ✅ → ~~TCS 206C(1H)~~ ✅ → ~~composition CMP-08~~ ✅ → cost centres UI → interest on overdue → budgets → FIFO valuation → stock ageing → GSP direct IRP/EWB APIs → WhatsApp docs → ratio analysis → post-dated vouchers | Steady parity grind; each item small and independent | IN PROGRESS — landed cost + TCS + CMP-08 done 2026-06-10 |
