...
| 6 | **Tally parity backlog** (doc §1 order): ~~landed cost~~ ✅ → ~~TCS 206C(1H)~~ ✅ → ~~composition CMP-08~~ ✅ → ~~cost centres UI~~ ✅ → ~~interest on overdue~~ ✅ → ~~stock ageing~~ ✅ → ~~ratio analysis~~ ✅ → ~~budgets~~ ✅ → FIFO valuation → GSP direct IRP/EWB APIs → WhatsApp docs → post-dated vouchers | Steady parity grind; read-only reports pulled forward | IN PROGRESS — 8 of 12 done 2026-06-10. Remaining 4 are large/external: FIFO (costing engine change), GSP (needs provider creds), WhatsApp templates, PDVs |
