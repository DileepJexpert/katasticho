...
| 6 | **Tally parity backlog** (doc §1 order): ~~landed cost~~ ✅ → ~~TCS 206C(1H)~~ ✅ → ~~composition CMP-08~~ ✅ → ~~cost centres UI~~ ✅ → ~~interest on overdue~~ ✅ → ~~stock ageing~~ ✅ → ~~ratio analysis~~ ✅ → ~~budgets~~ ✅ → ~~post-dated vouchers~~ ✅ → FIFO valuation → GSP direct IRP/EWB APIs → WhatsApp docs | Steady parity grind | IN PROGRESS — 9 of 12 done 2026-06-10. Remaining 3 are large/external: FIFO (costing engine change — dedicated session), GSP direct APIs (needs provider credentials), WhatsApp templates |
