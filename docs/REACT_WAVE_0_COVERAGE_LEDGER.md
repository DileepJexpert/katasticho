...
| W0-05 | Items, prices, tax, units, warehouses, stock masters | `/api/v1/items`, `/api/v1/stock/items/{itemId}/balances`, `/api/v1/stock/items/{itemId}/movements`, `/api/v1/batches/item/{itemId}`, `/api/v1/inventory/packaging-barcodes/items/{itemId}`, `/api/v1/warehouses`, `/api/v1/inventory/warehouse-zones/by-warehouse/{warehouseId}`, `/api/v1/price-lists`, `/api/v1/uoms` | Inventory group: item, warehouse, price-list, and units of measure directory/detail review routes | Inventory pack P0/P1 | React web | 2-3 | BUILDING: item directory/detail review plus typed create/edit now cover catalog identity, price, GST/HSN, base/purchase/secondary units, batch controls, preferred vendor, and audited create-only opening stock. Imports, serial operations, warehouse/zone/putaway, price-list/customer-tier, UoM mutation, and stock-execution mutations remain Flutter-only. |
