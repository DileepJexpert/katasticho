...
> Reconciliation 2026-09-05: this ledger retains early discovery notes. Use
> section 7 of REACT_WEB_MIGRATION_PLAN.md for current implementation status.
> HR/Payroll, Field Sales/MR, POS and several later-wave React workspaces now
> exist; they need parity review and acceptance, not wholesale reimplementation.
> No COMPLETE status follows merely from a page or implementation commit.

Latest source pass (2026-09-05): rack/putaway, UoM metadata, serial review,
stock-ledger paging/audit and file import are wired. Tax-group review was aligned
to active-only GET and OWNER/ADMIN/ACCOUNTANT/VIEWER access; no maintenance
...
| W0-03 | Dashboard and work queues | Dashboard, AR/AP reports, stock, workflow, field-sales dashboard APIs | Dashboard and accounting dashboard routes | Sales, Purchase, Inventory, Accounting P0 cases | React web | 1-3 | BUILDING: operations and dedicated accounting dashboards use existing server dashboard and financial-report queries; manual acceptance remains pending. |
...
| W0-05 | Items, prices, tax, units, warehouses, stock masters | `/api/v1/items`, `/api/v1/items/import/preview`, `/api/v1/items/import`, `/api/v1/stock/items/{itemId}/balances`, `/api/v1/stock/items/{itemId}/movements`, `/api/v1/batches/item/{itemId}`, `/api/v1/inventory/packaging-barcodes/items/{itemId}`, `/api/v1/warehouses`, `/api/v1/inventory/warehouse-zones/by-warehouse/{warehouseId}`, `/api/v1/price-lists`, `/api/v1/uoms` | Inventory group: item, warehouse, rack, price-list, and UoM review/maintenance; Items > Import | Inventory pack P0/P1 | React web | 2-3 | BUILDING: item create/edit/opening-stock, CSV/XLSX preview/commit import, warehouse/zone/rack maintenance, price-list/tier/customer-assignment operations and UoM metadata writes are source-wired with endpoint roles. Serial review and paged movement audit are available. Packaging mutations remain blocked after collision/uniqueness review; conversion rules have no API. Automated and runtime acceptance is not complete. |
...
| W0-07 | Order to cash | `/api/v1/sales-orders`, `/api/v1/delivery-challans`, `/api/v1/invoices`, `/api/v1/payments`, `/api/v1/customer-receipts`, `/api/v1/payment-terms` | Sales group: SO, challan, invoices, receipts, credit notes; Settings Payment Terms | Sales pack P0/P1 | React web | 2 | BUILDING: read-only, status-filtered Sales Order and Invoice directories plus document review use the current paginated list/detail contracts. Payment Terms provides a read-only instalment schedule review. All workflow and payment-term mutations remain Flutter-only. |
...
| W0-09 | Inventory execution and costing | `/api/v1/stock-counts`, `/api/v1/transfer-orders`, `/api/v1/picklists`, `/api/v1/batches`, `/api/v1/serial-numbers`, `/api/v1/inventory/*` | Inventory group: counts, transfers, picklists, putaway, serial review, barcode, consignment | Inventory pack P0/P1 | React web; scan/print certification required | 3 | BUILDING: reviewed picklists, Shortbook, batch-aware transfers, counts, stock reports, printer code and consignment settlement IDs. Added shared rack creation, putaway create/confirm/cancel, serial history/availability, and paged stock audit. Putaway is placement-only; consignment is register-only. Batch counts/adjustments, generic reversals, serial writes and controlled expiry returns retain frozen-contract/integration blockers. Tests/build, hardware and stock/GL acceptance remain pending; see REACT_INVENTORY_REVIEW_ACCEPTANCE.md. |
| W0-10 | Accounting, banking, audit, financial reports | `/api/v1/accounts`, `/api/v1/accounts/{id}/transactions`, `/api/v1/journal-entries`, `/api/v1/accounting/*`, `/api/v1/bank-*`, `/api/v1/reports` | Accounting, banking, reports groups | `TC-ACC-*` | React web | 3 | BUILDING: contract-verified read-only Chart of Accounts directory and account-ledger review are available. Account/chart-template writes and all journals, banking, and financial-report workflows remain Flutter-only. |
| W0-11 | GST, TDS/TCS, e-invoice, statutory compliance | `/api/v1/gst/*`, `/api/v1/tds`, `/api/v1/tcs`, `/api/v1/tax-groups`, tax-account/settings APIs | Tax & Compliance group | `TC-GST-*` | React web | 3 | BUILDING: contract-verified read-only Tax Groups directory and component-rate breakdown review are available alongside Tax Account Mappings. Tax group/rate modifications and all GST/TDS/TCS filing and compliance mutations remain Flutter-only. |
