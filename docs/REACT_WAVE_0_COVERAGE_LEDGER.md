...
| W0-05 | Items, prices, tax, units, warehouses, stock masters | `/api/v1/items`, `/api/v1/stock/items/{itemId}/balances`, `/api/v1/stock/items/{itemId}/movements`, `/api/v1/batches/item/{itemId}`, `/api/v1/inventory/packaging-barcodes/items/{itemId}`, `/api/v1/warehouses`, `/api/v1/inventory/warehouse-zones/by-warehouse/{warehouseId}`, `/api/v1/price-lists` | Inventory group and item/warehouse/price-list directory/detail review routes | Inventory pack P0/P1 | React web | 2-3 | BUILDING: contract-verified read-only Item, warehouse, and price-list review cover catalog/master directories; stock and warehouse facts; stock ledger; batch history; packaging; storage zones; price tiers; and customer assignments. Create/edit/import, serial, warehouse/zone/putaway, price-list/customer-tier, and stock-execution mutations remain Flutter-only. |
