...
| W0-02 | App shell, search, capability navigation, notifications | Organisation/settings, notifications, dashboard APIs | Stable sidebar IDs in `shell_screen.dart`; command/search shell | Cross-module role UAT | React web | 1 | BUILDING: shell and placeholder global search exist; capability navigation and notifications remain pending |
...
| W0-04 | Contacts, parties, supplier eligibility, statements | `/api/v1/contacts`, `/api/v1/suppliers`, `/api/v1/customer-receipts` | Standalone Contacts nav and create/detail/statement routes | `TC-SAL-001...`; `TC-PUR-001...004` | React web | 2 | BUILDING: read-only list uses role-safe backend filters; mutations and statements remain pending |
