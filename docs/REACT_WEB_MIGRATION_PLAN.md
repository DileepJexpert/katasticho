...
|---|---|---:|---|---|
| R-00 | OpenAPI contract and route/endpoint ledger | 0 | BUILDING | SpringDoc OpenAPI configuration and scoped security are unit-tested. Deterministic `contract:sync`, `contract:generate`, `contract:check`, and `contract:test` commands now validate the live document, write SHA-256 metadata, generate the endpoint ledger, and generate TypeScript types. The authoritative snapshot remains pending because Docker/PostgreSQL and Spring on port 8080 were unavailable on 2026-09-10; no contract is fabricated when the source server is offline. |
| R-01 | Browser auth, tenant, roles, capabilities, shell | 1 | QA | React consumes the existing web-session endpoint and keeps the access token in memory. Organisation switching now cancels in-flight requests and clears the prior tenant's complete query cache. Five Playwright browser checks cover anonymous redirect, form validation, authenticated shell entry, owner-only denial, and tenant-context replacement. Authoritative country/capability inputs and live backend browser acceptance remain pending. |
| R-02 | Design system and shared ERP primitives | 1 | QA | Token CSS and shared dense primitives are in broad use across the migrated pages. All 198 feature-page imports in the central router are now route-level lazy chunks behind one shared session-loading boundary; the production entry bundle fell from approximately 2.09 MB to 444.40 kB (133.85 kB gzip) on 2026-09-10. The named-export loader has focused regression tests, the full 483-test React suite passes, and the five browser shell checks pass with deterministic Playwright worker limits. Live visual acceptance at the required viewports remains open. |
...
| R-11 | Field Sales/MR administration | 4 | BUILDING | Planning, assignments, execution, GPS visit actions, collections, and day-close source contract-reviewed. Day-close resubmission for REJECTED status resolved in backend. MR/native execution and end-to-end acceptance remain pending. |
| R-12 | Pharma and Manufacturing | 4 | BUILDING | React pharmacy, BOM, work-order, QC, maintenance and related pages exist; BMR PDF exports now use authenticated browser downloads. Manufacturing is under concurrent work; review complete action loops and real item/batch effects before acceptance. |
