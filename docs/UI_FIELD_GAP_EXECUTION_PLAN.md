...
- [x] **B2. Name resolution — pattern established + SCM cases done.** Added a `@Transient` name field on `SupplierPerformance` (`supplierName`) and `ReorderPolicy` (`itemName`) + cached resolve helpers in `SupplyChainService` (no migration — transient fields aren't validated by Hibernate). Supplier rankings now show the real supplier name (was the literal `'Supplier'`); ABC list shows item names (was a UUID prefix). **Backend compiles clean.** **Rollout TODO** (same transient-name + resolve-helper pattern): Manufacturing WO BOM-lines item names, job-card operation/workstation/assignee names, QC inspection item/parameter/inspector names — best done alongside the B1 EntityPicker rollout since they touch the same screens.
