...
class _JobWorkDetailScreenState extends ConsumerState<JobWorkDetailScreen> {
  ...
  Future<void> _showReceiveDialog(Map<String, dynamic> order) async {
    ...
    for (final line in lines) {
      final itemId = line['itemId']?.toString() ?? '';
      // Controllers already disposed — we captured values before disposal won't work
      // We need to read before disposing. Refactor: read during dialog close instead.
      // Since we disposed them above, we use the last known text. This is a limitation
      // of dispose-before-read. For correctness, capture values in onPressed closure:
      // (This code path won't be reached after the controller fix below.)
    }
...
// ---------------------------------------------------------------------------
// Receive dialog — proper implementation using StatefulBuilder
// ---------------------------------------------------------------------------

Future<void> _showReceiveDialogProper({
  ...
  if (confirmed != true) {
    for (final ctls in controllers.values) {
      ctls.received.dispose();
      ctls.wastage.dispose();
    }
    return;
  ...
  // Build payload before disposing
  final receiptLines = <Map<String, dynamic>>[];
  for (final line in lines) {
    final itemId = line['itemId']?.toString() ?? '';
    final ctls = controllers[itemId];
    if (ctls == null) continue;
    final receivedQty = double.tryParse(ctls.received.text.trim()) ?? 0;
    final wastageQty = double.tryParse(ctls.wastage.text.trim()) ?? 0;
    receiptLines.add({
      'itemId': itemId,
  ...
  try {
    await ref
        .read(manufacturingRepositoryProvider)
        .receiveJobWorkGoods(jobWorkId, receiptLines);
    onSuccess();
  } catch (e) {
    if (context.mounted) {
      ScaffoldMessenger.of(context)
