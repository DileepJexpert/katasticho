...
class PriceListListScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return KKeyboardListWrapper(
      itemCount: () => listsAsync.valueOrNull?.length ?? 0,
      onNew: () => context.go('/price-lists/create'),
      onRefresh: () => ref.invalidate(priceListsProvider),
      onOpen: (index) {
        final lists = listsAsync.valueOrNull;
        if (lists != null && index < lists.length) {
          final id = lists[index]['id']?.toString() ?? '';
