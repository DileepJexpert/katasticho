...
export function BatchAllocationPicker({ itemId, value, onChange, warehouseId, warehouseName, quantity, automatic = false, disabled = false }: {
  itemId: string | null; value: string | null; onChange: (batchId: string | undefined, batch?: BatchDetail) => void
  warehouseId?: string | null; warehouseName?: string | null; quantity: number; automatic?: boolean; disabled?: boolean
}) {
  ...
  return <>
    ...
    {open && <Modal isOpen size="lg" title="Batch allocation" onClose={() => setOpen(false)}
      ...
      footer={<><Button variant="secondary" onClick={() => setOpen(false)}>Close</Button>{!automatic && value && <Button variant="secondary" onClick={() => choose()}>Clear batch selection</Button>}{automatic && <Button onClick={() => choose()}>Use automatic FEFO</Button>}</>}>
      {item.isPending || (item.data?.trackBatches && batches.isPending) ? <div className="directory-state" role="status">Loading available batches...</div>
        : item.isError || batches.isError ? <Button onClick={() => { void item.refetch(); if (item.data?.trackBatches) void batches.refetch() }} variant="secondary">Retry</Button>
          : !item.data?.trackBatches ? <p>This item does not use batch tracking.</p> : <>
            ...
            {!automatic && <p className="cell-muted">Select a batch to record this challan's batch-level stock movement. This dispatch does not automatically split the line across batches.</p>}
