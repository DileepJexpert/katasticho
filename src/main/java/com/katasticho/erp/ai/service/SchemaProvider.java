...
public class SchemaProvider {
    ...
    public String getSchemaDescription() {
        return """
                ...
                -- Contacts (unified: customers, vendors, both)
                contact (id UUID PK, org_id UUID, contact_type VARCHAR [CUSTOMER/VENDOR/BOTH], display_name VARCHAR, company_name VARCHAR, gstin VARCHAR, pan VARCHAR, gst_treatment VARCHAR, billing_address_line1 TEXT, billing_city VARCHAR, billing_state VARCHAR, billing_state_code VARCHAR, billing_postal_code VARCHAR, credit_limit DECIMAL, payment_terms_days INT, is_deleted BOOLEAN, created_at TIMESTAMP)

                -- Invoices (AR)
                invoice (id UUID PK, org_id UUID, invoice_number VARCHAR, contact_id UUID FK, invoice_date DATE, due_date DATE, status VARCHAR [DRAFT/SENT/PARTIALLY_PAID/PAID/CANCELLED/OVERDUE], currency VARCHAR, subtotal DECIMAL, tax_total DECIMAL, total DECIMAL, amount_paid DECIMAL, balance_due DECIMAL, base_subtotal DECIMAL, base_tax_total DECIMAL, base_total DECIMAL, journal_entry_id UUID, notes TEXT, created_at TIMESTAMP)
                ...
                -- Payments
                payment (id UUID PK, org_id UUID, contact_id UUID FK, invoice_id UUID FK, payment_number VARCHAR, payment_date DATE, amount DECIMAL, currency VARCHAR, base_amount DECIMAL, payment_method VARCHAR [CASH/BANK_TRANSFER/UPI/CHEQUE/CARD/OTHER], reference_number VARCHAR, journal_entry_id UUID, created_at TIMESTAMP)

                -- Credit Notes
                credit_note (id UUID PK, org_id UUID, credit_note_number VARCHAR, contact_id UUID FK, invoice_id UUID, issue_date DATE, status VARCHAR [DRAFT/ISSUED/APPLIED/CANCELLED], subtotal DECIMAL, tax_total DECIMAL, total DECIMAL, reason TEXT, journal_entry_id UUID, created_at TIMESTAMP)
                ...
                - credit_note.contact_id → contact.id
