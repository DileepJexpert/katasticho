package com.katasticho.erp.hr.repository;

import com.katasticho.erp.hr.entity.EmployeeFamily;
import org.springframework.data.jpa.repository.JpaRepository;
import org.springframework.stereotype.Repository;

import java.util.List;
import java.util.Optional;
import java.util.UUID;

@Repository
public interface EmployeeFamilyRepository extends JpaRepository<EmployeeFamily, UUID> {

    Optional<EmployeeFamily> findByIdAndOrgIdAndIsDeletedFalse(UUID id, UUID orgId);

    List<EmployeeFamily> findByOrgIdAndEmployeeIdAndIsDeletedFalseOrderByCreatedAtAsc(
            UUID orgId, UUID employeeId);
}
