    ...
    public ItemResponse createItem(CreateItemRequest request) {
        ...
        if (trackInventory && openingStock != null && openingStock.compareTo(BigDecimal.ZERO) > 0) {
            ...
            if (item.isTrackBatches()) {
                if (request.openingBatchNumber() == null || request.openingBatchNumber().isBlank()) {
                    throw new BusinessException(
                            "Batch number is required for opening stock on batch-tracked items",
                            "INV_OPENING_BATCH_REQUIRED", HttpStatus.BAD_REQUEST);
                }
                StockBatch batch = batchService.upsertBatch(
                        item.getId(),
                        request.openingBatchNumber().trim(),
                        request.openingExpiryDate(),
                        request.openingMfgDate(),
