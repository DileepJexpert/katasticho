-- ============================================================
-- Katasticho ERP — Consolidated baseline schema (V1)
--
-- Re-baselined 2026-06-12 from the fully-migrated database
-- (previous migrations V1..V70 squashed; databases created before
-- this baseline must be dropped and recreated).
--
-- Conventions: org-scoped tables carry org_id/is_deleted/audit columns;
-- platform reference tables (drug_master, salt_master, hsn_gst_master,
-- manufacturer_master, currency, coa_template, ...) have no org_id.
-- ============================================================
...
CREATE TABLE public.account (
    ...
    CONSTRAINT account_type_check CHECK (((type)::text = ANY ((ARRAY['ASSET'::character varying, 'LIABILITY'::character varying, 'EQUITY'::character varying, 'REVENUE'::character varying, 'EXPENSE'::character varying])::text[])))
);
...
CREATE TABLE public.app_user (
    ...
    CONSTRAINT app_user_role_check CHECK (((role)::text = ANY ((ARRAY['OWNER'::character varying, 'ADMIN'::character varying, 'ACCOUNTANT'::character varying, 'OPERATOR'::character varying, 'VIEWER'::character varying, 'CA_EXTERNAL'::character varying, 'CA_PARTNER'::character varying, 'CA_STAFF'::character varying, 'PLATFORM_ADMIN'::character varying])::text[]))),
...
CREATE TABLE public.ca_client_link (
    ...
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    created_by uuid,
    CONSTRAINT ca_client_link_status_check CHECK (((status)::text = ANY ((ARRAY['ACTIVE'::character varying, 'PAUSED'::character varying, 'ENDED'::character varying])::text[])))
);
...
CREATE TABLE public.ca_compliance_deadline (
    ...
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT ca_compliance_deadline_status_check CHECK (((status)::text = ANY ((ARRAY['PENDING'::character varying, 'FILED'::character varying, 'OVERDUE'::character varying, 'NOT_APPLICABLE'::character varying])::text[])))
);
...
CREATE TABLE public.ca_report_dispatch (
    ...
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT ca_report_dispatch_sent_via_check CHECK (((sent_via)::text = ANY ((ARRAY['EMAIL'::character varying, 'WHATSAPP'::character varying, 'BOTH'::character varying])::text[]))),
    CONSTRAINT ca_report_dispatch_status_check CHECK (((status)::text = ANY ((ARRAY['QUEUED'::character varying, 'SENT'::character varying, 'FAILED'::character varying])::text[])))
);
...
CREATE TABLE public.contact (
    ...
    CONSTRAINT contact_contact_type_check CHECK (((contact_type)::text = ANY ((ARRAY['CUSTOMER'::character varying, 'VENDOR'::character varying, 'BOTH'::character varying])::text[]))),
    CONSTRAINT contact_gst_treatment_check CHECK (((gst_treatment)::text = ANY ((ARRAY['REGISTERED'::character varying, 'UNREGISTERED'::character varying, 'COMPOSITION'::character varying, 'CONSUMER'::character varying, 'OVERSEAS'::character varying, 'SEZ'::character varying])::text[])))
);
...
CREATE TABLE public.delivery_challan (
    ...
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    created_by uuid,
    CONSTRAINT delivery_challan_status_check CHECK (((status)::text = ANY ((ARRAY['DRAFT'::character varying, 'DISPATCHED'::character varying, 'DELIVERED'::character varying, 'CANCELLED'::character varying])::text[])))
);
...
CREATE TABLE public.drug_interaction (
    ...
    CONSTRAINT chk_drug_interaction_severity CHECK (((severity)::text = ANY ((ARRAY['LOW'::character varying, 'MODERATE'::character varying, 'HIGH'::character varying, 'CRITICAL'::character varying])::text[])))
);
...
CREATE TABLE public.employee_salary_component (
    ...
    created_at timestamp with time zone DEFAULT now(),
    CONSTRAINT employee_salary_component_calculation_type_check CHECK (((calculation_type)::text = ANY ((ARRAY['FIXED'::character varying, 'PERCENTAGE'::character varying, 'FORMULA'::character varying])::text[])))
);
...
CREATE TABLE public.estimate (
    ...
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    created_by uuid,
    CONSTRAINT estimate_status_check CHECK (((status)::text = ANY ((ARRAY['DRAFT'::character varying, 'SENT'::character varying, 'ACCEPTED'::character varying, 'DECLINED'::character varying, 'INVOICED'::character varying, 'EXPIRED'::character varying])::text[])))
);
...
CREATE TABLE public.expense (
    ...
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    created_by uuid,
    CONSTRAINT expense_payment_mode_check CHECK (((payment_mode)::text = ANY ((ARRAY['CASH'::character varying, 'BANK'::character varying, 'UPI'::character varying, 'CREDIT_CARD'::character varying])::text[]))),
    CONSTRAINT expense_status_check CHECK (((status)::text = ANY ((ARRAY['RECORDED'::character varying, 'BILLABLE'::character varying, 'INVOICED'::character varying, 'VOID'::character varying])::text[])))
);
...
CREATE TABLE public.fiscal_period (
    ...
    CONSTRAINT fiscal_period_status_check CHECK (((status)::text = ANY ((ARRAY['OPEN'::character varying, 'CLOSED'::character varying, 'LOCKED'::character varying])::text[])))
);
...
CREATE TABLE public.invoice (
    ...
    CONSTRAINT invoice_status_check CHECK (((status)::text = ANY ((ARRAY['DRAFT'::character varying, 'SENT'::character varying, 'PARTIALLY_PAID'::character varying, 'PAID'::character varying, 'CANCELLED'::character varying, 'OVERDUE'::character varying])::text[])))
);
...
CREATE TABLE public.item (
    ...
    CONSTRAINT item_item_type_check CHECK (((item_type)::text = ANY ((ARRAY['GOODS'::character varying, 'SERVICE'::character varying, 'COMPOSITE'::character varying])::text[])))
);
...
CREATE TABLE public.journal_entry (
    ...
    CONSTRAINT journal_entry_approval_status_check CHECK (((approval_status)::text = ANY ((ARRAY['NONE'::character varying, 'PENDING'::character varying, 'APPROVED'::character varying, 'REJECTED'::character varying])::text[]))),
    CONSTRAINT journal_entry_period_month_check CHECK (((period_month >= 1) AND (period_month <= 12))),
    CONSTRAINT journal_entry_source_module_check CHECK (((source_module)::text = ANY ((ARRAY['SALES'::character varying, 'PURCHASE'::character varying, 'PAYMENT'::character varying, 'PAYROLL'::character varying, 'INVENTORY'::character varying, 'MANUAL'::character varying, 'GST'::character varying, 'BANK_REC'::character varying, 'OPENING'::character varying, 'POS'::character varying, 'EXPENSE'::character varying, 'MANUFACTURING'::character varying, 'HOSPITAL'::character varying])::text[]))),
    CONSTRAINT journal_entry_status_check CHECK (((status)::text = ANY ((ARRAY['DRAFT'::character varying, 'POSTED'::character varying])::text[])))
);
...
CREATE TABLE public.network_order (
    ...
    cancelled_at timestamp with time zone,
    is_deleted boolean DEFAULT false NOT NULL,
    CONSTRAINT chk_network_order_status CHECK (((status)::text = ANY ((ARRAY['PLACED'::character varying, 'CONFIRMED'::character varying, 'PARTIALLY_CONFIRMED'::character varying, 'REJECTED'::character varying, 'PROCESSING'::character varying, 'DISPATCHED'::character varying, 'PARTIALLY_DISPATCHED'::character varying, 'DELIVERED'::character varying, 'PARTIALLY_DELIVERED'::character varying, 'CANCELLED'::character varying, 'CLOSED'::character varying])::text[])))
);
...
CREATE TABLE public.network_order_line (
    ...
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT chk_nol_status CHECK (((status)::text = ANY ((ARRAY['PENDING'::character varying, 'CONFIRMED'::character varying, 'REJECTED'::character varying, 'DISPATCHED'::character varying, 'DELIVERED'::character varying, 'CANCELLED'::character varying])::text[])))
);
...
CREATE TABLE public.notification (
    ...
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT notification_channel_check CHECK (((channel)::text = ANY ((ARRAY['IN_APP'::character varying, 'EMAIL'::character varying, 'WHATSAPP'::character varying, 'SMS'::character varying, 'PUSH'::character varying])::text[]))),
    CONSTRAINT notification_severity_check CHECK (((severity)::text = ANY ((ARRAY['INFO'::character varying, 'WARNING'::character varying, 'CRITICAL'::character varying])::text[]))),
    CONSTRAINT notification_type_check CHECK (((type)::text = ANY ((ARRAY['PAYMENT_REMINDER'::character varying, 'EXPIRY_ALERT'::character varying, 'LOW_STOCK_ALERT'::character varying, 'DAILY_SUMMARY'::character varying, 'BILL_OVERDUE'::character varying, 'SYSTEM'::character varying, 'INFO'::character varying, 'WARNING'::character varying])::text[])))
);
...
CREATE TABLE public.org_ai_settings (
    ...
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    updated_by uuid,
    CONSTRAINT org_ai_settings_provider_check CHECK (((provider)::text = ANY ((ARRAY['CLAUDE'::character varying, 'OLLAMA'::character varying])::text[])))
);
...
CREATE TABLE public.organisation (
    ...
    CONSTRAINT organisation_approval_status_check CHECK (((approval_status)::text = ANY ((ARRAY['PENDING'::character varying, 'APPROVED'::character varying, 'REJECTED'::character varying, 'SUSPENDED'::character varying])::text[]))),
    CONSTRAINT organisation_business_type_check CHECK (((business_type)::text = ANY ((ARRAY['RETAILER'::character varying, 'DISTRIBUTOR'::character varying, 'MANUFACTURER'::character varying, 'SERVICE_PROVIDER'::character varying])::text[])))
);
...
CREATE TABLE public.payment (
    ...
    voided_by uuid,
    CONSTRAINT payment_payment_method_check CHECK (((payment_method)::text = ANY ((ARRAY['CASH'::character varying, 'BANK_TRANSFER'::character varying, 'UPI'::character varying, 'CHEQUE'::character varying, 'CARD'::character varying, 'OTHER'::character varying])::text[]))),
    CONSTRAINT payment_status_check CHECK (((status)::text = ANY ((ARRAY['DRAFT'::character varying, 'POSTED'::character varying, 'PENDING_APPROVAL'::character varying, 'VOIDED'::character varying])::text[])))
);
...
CREATE TABLE public.payroll_run (
    ...
    created_at timestamp with time zone DEFAULT now(),
    updated_at timestamp with time zone DEFAULT now(),
    CONSTRAINT payroll_run_status_check CHECK (((status)::text = ANY ((ARRAY['DRAFT'::character varying, 'CALCULATED'::character varying, 'APPROVED'::character varying, 'POSTED'::character varying, 'CANCELLED'::character varying])::text[])))
);
...
CREATE TABLE public.picklist (
    ...
    created_by uuid,
    is_deleted boolean DEFAULT false NOT NULL,
    CONSTRAINT picklist_status_check CHECK (((status)::text = ANY ((ARRAY['PENDING'::character varying, 'IN_PROGRESS'::character varying, 'COMPLETED'::character varying, 'CANCELLED'::character varying])::text[])))
);
...
CREATE TABLE public.published_catalog_item (
    ...
    created_by uuid,
    is_deleted boolean DEFAULT false NOT NULL,
    CONSTRAINT chk_catalog_availability CHECK (((availability_status)::text = ANY ((ARRAY['AVAILABLE'::character varying, 'LOW_STOCK'::character varying, 'OUT_OF_STOCK'::character varying, 'DISCONTINUED'::character varying])::text[])))
);
...
CREATE TABLE public.purchase_bill (
    ...
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    created_by uuid,
    CONSTRAINT purchase_bill_status_check CHECK (((status)::text = ANY ((ARRAY['DRAFT'::character varying, 'OPEN'::character varying, 'PARTIALLY_PAID'::character varying, 'PAID'::character varying, 'VOID'::character varying, 'OVERDUE'::character varying])::text[])))
);
...
CREATE TABLE public.recurring_invoice (
    ...
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    created_by uuid,
    CONSTRAINT recurring_invoice_frequency_check CHECK (((frequency)::text = ANY ((ARRAY['WEEKLY'::character varying, 'MONTHLY'::character varying, 'QUARTERLY'::character varying, 'HALF_YEARLY'::character varying, 'YEARLY'::character varying])::text[]))),
    CONSTRAINT recurring_invoice_status_check CHECK (((status)::text = ANY ((ARRAY['ACTIVE'::character varying, 'PAUSED'::character varying, 'STOPPED'::character varying, 'EXPIRED'::character varying])::text[])))
);
...
CREATE TABLE public.salary_component (
    ...
    created_at timestamp with time zone DEFAULT now(),
    CONSTRAINT salary_component_component_type_check CHECK (((component_type)::text = ANY ((ARRAY['EARNING'::character varying, 'DEDUCTION'::character varying, 'EMPLOYER_CONTRIBUTION'::character varying])::text[])))
);
...
CREATE TABLE public.sales_order (
    ...
    currency_code character varying(3) DEFAULT 'INR'::character varying NOT NULL,
    exchange_rate numeric(15,6) DEFAULT 1 NOT NULL,
    CONSTRAINT sales_order_discount_type_check CHECK (((discount_type)::text = ANY ((ARRAY['ITEM_LEVEL'::character varying, 'ENTITY_LEVEL'::character varying])::text[]))),
    CONSTRAINT sales_order_invoiced_status_check CHECK (((invoiced_status)::text = ANY ((ARRAY['NOT_INVOICED'::character varying, 'PARTIALLY_INVOICED'::character varying, 'FULLY_INVOICED'::character varying])::text[]))),
    CONSTRAINT sales_order_shipped_status_check CHECK (((shipped_status)::text = ANY ((ARRAY['NOT_SHIPPED'::character varying, 'PARTIALLY_SHIPPED'::character varying, 'FULLY_SHIPPED'::character varying])::text[]))),
    CONSTRAINT sales_order_status_check CHECK (((status)::text = ANY ((ARRAY['DRAFT'::character varying, 'CONFIRMED'::character varying, 'BACKORDER'::character varying, 'PARTIALLY_SHIPPED'::character varying, 'SHIPPED'::character varying, 'PARTIALLY_INVOICED'::character varying, 'INVOICED'::character varying, 'COMPLETED'::character varying, 'CANCELLED'::character varying, 'VOID'::character varying])::text[])))
);
...
CREATE TABLE public.sales_receipt (
    ...
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    created_by uuid,
    CONSTRAINT sales_receipt_payment_mode_check CHECK (((payment_mode)::text = ANY ((ARRAY['CASH'::character varying, 'UPI'::character varying, 'CARD'::character varying, 'MIXED'::character varying])::text[])))
);
...
CREATE TABLE public.serial_number (
    ...
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    created_by uuid,
    CONSTRAINT serial_number_status_check CHECK (((status)::text = ANY ((ARRAY['IN_STOCK'::character varying, 'SOLD'::character varying, 'DAMAGED'::character varying, 'RETURNED'::character varying, 'RESERVED'::character varying])::text[])))
);
...
CREATE TABLE public.stock_count (
    ...
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    created_by uuid,
    CONSTRAINT stock_count_status_check CHECK (((status)::text = ANY ((ARRAY['DRAFT'::character varying, 'POSTED'::character varying, 'CANCELLED'::character varying])::text[])))
);
...
CREATE TABLE public.stock_movement (
    ...
    notes text,
    created_by uuid,
    CONSTRAINT stock_movement_movement_type_check CHECK (((movement_type)::text = ANY ((ARRAY['PURCHASE'::character varying, 'SALE'::character varying, 'ADJUSTMENT'::character varying, 'TRANSFER_IN'::character varying, 'TRANSFER_OUT'::character varying, 'OPENING'::character varying, 'RETURN_IN'::character varying, 'RETURN_OUT'::character varying, 'STOCK_COUNT'::character varying, 'REVERSAL'::character varying])::text[]))),
    CONSTRAINT stock_movement_reference_type_check CHECK (((reference_type)::text = ANY ((ARRAY['INVOICE'::character varying, 'CREDIT_NOTE'::character varying, 'BILL'::character varying, 'DEBIT_NOTE'::character varying, 'STOCK_ADJUSTMENT'::character varying, 'STOCK_TRANSFER'::character varying, 'STOCK_COUNT'::character varying, 'OPENING_BALANCE'::character varying, 'STOCK_RECEIPT'::character varying, 'DELIVERY_CHALLAN'::character varying, 'SALES_RECEIPT'::character varying])::text[])))
);
...
CREATE TABLE public.stock_receipt (
    ...
    CONSTRAINT stock_receipt_status_check CHECK (((status)::text = ANY ((ARRAY['DRAFT'::character varying, 'RECEIVED'::character varying, 'CANCELLED'::character varying])::text[])))
);
...
CREATE TABLE public.stock_reservation (
    ...
    cancelled_at timestamp with time zone,
    CONSTRAINT stock_reservation_source_type_check CHECK (((source_type)::text = ANY ((ARRAY['SALES_ORDER'::character varying, 'TRANSFER_ORDER'::character varying])::text[]))),
    CONSTRAINT stock_reservation_status_check CHECK (((status)::text = ANY ((ARRAY['ACTIVE'::character varying, 'FULFILLED'::character varying, 'CANCELLED'::character varying])::text[])))
);
...
CREATE TABLE public.tax_line_item (
    ...
    base_tax_amount numeric(15,2) DEFAULT 0 NOT NULL,
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT tax_line_item_source_type_check CHECK (((source_type)::text = ANY ((ARRAY['INVOICE'::character varying, 'CREDIT_NOTE'::character varying, 'BILL'::character varying, 'EXPENSE'::character varying, 'VENDOR_CREDIT'::character varying, 'SALES_RECEIPT'::character varying])::text[])))
);
...
CREATE TABLE public.tax_rate (
    ...
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT tax_rate_tax_type_check CHECK (((tax_type)::text = ANY ((ARRAY['OUTPUT'::character varying, 'INPUT'::character varying, 'BOTH'::character varying])::text[])))
);
...
CREATE TABLE public.trading_partner (
    ...
    CONSTRAINT chk_trading_partner_status CHECK (((status)::text = ANY ((ARRAY['PENDING'::character varying, 'APPROVED'::character varying, 'REJECTED'::character varying, 'SUSPENDED'::character varying, 'REVOKED'::character varying])::text[])))
);
...
CREATE TABLE public.transfer_order (
    ...
    CONSTRAINT transfer_order_status_check CHECK (((status)::text = ANY ((ARRAY['DRAFT'::character varying, 'IN_TRANSIT'::character varying, 'RECEIVED'::character varying, 'CANCELLED'::character varying])::text[])))
);
...
CREATE TABLE public.uom (
    ...
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    created_by uuid,
    CONSTRAINT uom_category_check CHECK (((category)::text = ANY ((ARRAY['WEIGHT'::character varying, 'VOLUME'::character varying, 'COUNT'::character varying, 'LENGTH'::character varying, 'PACKAGING'::character varying])::text[])))
);
...
CREATE TABLE public.user_invitation (
    ...
    CONSTRAINT user_invitation_role_check CHECK (((role)::text = ANY ((ARRAY['OWNER'::character varying, 'ADMIN'::character varying, 'ACCOUNTANT'::character varying, 'OPERATOR'::character varying, 'VIEWER'::character varying, 'CA_EXTERNAL'::character varying, 'CA_PARTNER'::character varying, 'CA_STAFF'::character varying, 'PLATFORM_ADMIN'::character varying])::text[])))
);
...
CREATE TABLE public.vendor_credit (
    ...
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    created_by uuid,
    CONSTRAINT vendor_credit_status_check CHECK (((status)::text = ANY ((ARRAY['DRAFT'::character varying, 'OPEN'::character varying, 'APPLIED'::character varying, 'VOID'::character varying])::text[])))
);
...
CREATE TABLE public.vendor_payment (
    ...
    CONSTRAINT vendor_payment_payment_mode_check CHECK (((payment_mode)::text = ANY ((ARRAY['CASH'::character varying, 'BANK_TRANSFER'::character varying, 'UPI'::character varying, 'CHEQUE'::character varying, 'CARD'::character varying, 'OTHER'::character varying])::text[])))
);
...
CREATE TABLE public.work_order (
    ...
    CONSTRAINT work_order_priority_check CHECK (((priority)::text = ANY ((ARRAY['URGENT'::character varying, 'HIGH'::character varying, 'NORMAL'::character varying, 'LOW'::character varying])::text[])))
);
...
CREATE INDEX idx_invoice_org_due ON public.invoice USING btree (org_id, due_date) WHERE ((status)::text = ANY ((ARRAY['SENT'::character varying, 'PARTIALLY_PAID'::character varying, 'OVERDUE'::character varying])::text[]));
...
CREATE INDEX idx_purchase_bill_org_due ON public.purchase_bill USING btree (org_id, due_date) WHERE ((status)::text = ANY ((ARRAY['OPEN'::character varying, 'PARTIALLY_PAID'::character varying, 'OVERDUE'::character varying])::text[]));
