-- V1: Consolidated baseline schema (squashed from former V1-V71 on 2026-06-12).
-- Generated by applying every historical migration to PostgreSQL 16 and
-- dumping the resulting schema, so this file is guaranteed to match the
-- schema the old chain produced. CREATE statements only - no ALTERs.
-- Latent bugs fixed during consolidation: V59 inserted into a non-existent
-- account.system column (now is_system); V62's exchange_rate collided with
-- the V1 platform-level table (org-scoped V62 shape kept, matching the JPA
-- entity); V62's currency-column guards checked the wrong column.
...
CREATE TABLE public.journal_entry (
    ...
    CONSTRAINT journal_entry_source_module_check CHECK (((source_module)::text = ANY ((ARRAY['SALES'::character varying, 'PURCHASE'::character varying, 'PAYMENT'::character varying, 'PAYROLL'::character varying, 'INVENTORY'::character varying, 'MANUAL'::character varying, 'GST'::character varying, 'BANK_REC'::character varying, 'OPENING'::character varying, 'POS'::character varying, 'EXPENSE'::character varying])::text[]))),
